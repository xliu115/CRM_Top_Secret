---
name: Nudge Requirements Update
overview: Update the nudge requirements document to reflect all implemented features and architectural changes, including nudge consolidation, meeting briefs, partner relationship context, LinkedIn Activity support, and the new consolidated card UI.
todos:
  - id: update-plan-md
    content: "Rewrite the nudge_type_action_requirements plan.md to reflect all implemented changes: consolidation, meeting briefs, partner context, LinkedIn, updated labels, completed phases"
    status: completed
isProject: false
---
...
Update [nudge_type_action_requirements_dec96109.plan.md](/Users/Xinyu_Liu/.cursor/plans/nudge_type_action_requirements_dec96109.plan.md) to reflect the current implemented state. All original phases are now complete, plus significant architectural changes were made beyond the original scope.
...
- Key files: [nudge-engine.ts](src/lib/services/nudge-engine.ts) `Insight` interface, `pickPrimary()`, `buildReason()`
