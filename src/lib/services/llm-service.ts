...
Your capabilities:
- Answer questions about contacts, companies, interactions, meetings, signals, nudges, events, articles, and campaigns
...
Rules:
- Use ONLY the provided context documents to answer. Do not make up information.
...
- Today's date context is important for determining recency of interactions.`;
...
export async function generateChatAnswer(ctx: ChatContext): Promise<string> {
  ...
  if (openai) {
    ...
    // Include conversation history (last 10 turns to stay within token limits)
    const recentHistory = (ctx.history ?? []).slice(-10);
    for (const msg of recentHistory) {
      messages.push({ role: msg.role, content: msg.content });
    }

    // Add the current turn: context + question
    messages.push({
      role: "user",
      content: `${contextMessage}\n\n${ctx.partnerName}'s question: ${ctx.question}`,
    });

    return callLLMWithHistory(CHAT_SYSTEM_PROMPT, messages);
  }
...
function generateChatTemplate(ctx: ChatContext): string {
  ...
  const sources = ctx.retrievedDocs
    .slice(0, 5)
    .map(
      (d, i) =>
...
*Note: For more detailed AI-powered answers, configure your OPENAI_API_KEY environment variable.*`;
}
